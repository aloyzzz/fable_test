...
uniform float uMS;         // multiple-scattering fudge
...
vec3 inscatter(vec3 o, vec3 d, float tmax, out vec3 T) {
  ...
  vec3 sRs = vec3(0.0), sMs = vec3(0.0), sRm = vec3(0.0), sMm = vec3(0.0); vec3 od = vec3(0.0);
  for (int i = 0; i < N; i++) {
    ...
    if (ls.x >= 0.0) { vec3 att = exp(-extinction(od + ls)); sRs += att * dh.x; sMs += att * dh.y; }
    vec3 lm = lightDepth(p, uMoonDir);
    if (lm.x >= 0.0) { vec3 att = exp(-extinction(od + lm)); sRm += att * dh.x; sMm += att * dh.y; }
  }
  T = exp(-extinction(od));
...
export function skyRadianceJS(d, P, out, T, N = 10) {
  ...
  const ds = tmax / N;
  let sRs0 = 0, sRs1 = 0, sRs2 = 0, sMs0 = 0, sMs1 = 0, sMs2 = 0, sRm0 = 0, sRm1 = 0, sRm2 = 0, sMm0 = 0, sMm1 = 0, sMm2 = 0;
  _od[0] = _od[1] = _od[2] = 0;
  for (let i = 0; i < N; i++) {
    const t = (i + 0.5) * ds;
  ...
  out[0] = (P.sunE * (BETA_R[0] * prs * sRs0 + P.betaM * pms * sMs0) + P.moonE[0] * (BETA_R[0] * prm * sRm0 + P.betaM * pmm * sMm0)) * P.ms;
  out[1] = (P.sunE * (BETA_R[1] * prs * sRs1 + P.betaM * pms * sMs1) + P.moonE[1] * (BETA_R[1] * prm * sRm1 + P.betaM * pmm * sMm1)) * P.ms;
  out[2] = (P.sunE * (BETA_R[2] * prs * sRs2 + P.betaM * pms * sMs2) + P.moonE[2] * (BETA_R[2] * prm * sRm2 + P.betaM * pmm * sMm2)) * P.ms;
  return out;
}
